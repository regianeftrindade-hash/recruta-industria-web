...
function PagamentoEmpresa() {
  ...
  const startPolling = (chargeId: string, isBoleto: boolean) => {
    ...
    pollRef.current = setInterval(async () => {
      ...
      if (pollCountRef.current >= POLL_MAX) {
        ...
        setStatusMessage(
          isBoleto
            ? "Ainda aguardando o boleto. Pode fechar esta página; quando o pagamento for confirmado, o plano é ativado automaticamente. Use “Verificar agora” se quiser checar de novo."
  ...
  const handleCheckout = async () => {
    ...
    try {
      const payMethod =
        recurringSupported && billingMode === "recurring" ? "boleto" : method;
