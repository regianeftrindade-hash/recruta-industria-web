...
export async function activateCompanyPlanFromPayment(
  ...
  const sameTier = company?.planTier === planTier;
  const alreadyActive =
    sameTier && !expired && !options?.isRenewal && !meta?.isRenewal;

  if (!alreadyActive || options?.isRenewal || meta?.isRenewal || expired) {
    await setCompanyPlanTier(companyUserId, planTier, {
      ...
      extendFromCurrent: Boolean(options?.isRenewal || meta?.isRenewal) && !expired,
    });
  }
...
export async function activateCompanyExtraSeatsFromPayment(
  companyUserId: string,
  chargeId: string,
): Promise<{ alreadyActive: boolean; extraSeats: number }> {
  const { addCompanyExtraSeats, calcExtraSeatsAmountCentavos } = await import(
    '@/lib/company/company-extra-seats'
  );
  const { asCompanyExtraSeatsPaymentMeta } = await import('@/lib/payment-config');
  ...
  const meta = asCompanyExtraSeatsPaymentMeta(payment.meta);
  if (!meta) {
    throw new Error('Cobrança sem vínculo de usuário extra');
  }
  if (meta.companyUserId !== companyUserId) {
    throw new Error('Cobrança não pertence a esta empresa');
  }
  if (meta.activatedAt) {
    const { getCompanyExtraSeats } = await import('@/lib/company/company-extra-seats');
    return { alreadyActive: true, extraSeats: await getCompanyExtraSeats(companyUserId) };
  }

  const expected = calcExtraSeatsAmountCentavos(meta.quantity);
  if (expected !== Math.round(payment.amount) || meta.expectedAmount !== expected) {
    throw new Error('Valor pago não confere com o usuário extra');
  }

  const extraSeats = await addCompanyExtraSeats(companyUserId, meta.quantity);
  const nextMeta = { ...meta, activatedAt: new Date().toISOString() };
  await prisma.paymentRecord.update({
    where: { id: payment.id },
    data: { meta: JSON.stringify(nextMeta) },
  });

  return { alreadyActive: false, extraSeats };
}

export function buildCompanyExtraSeatDescription(quantity: number): string {
  const q = Math.max(1, Math.floor(quantity));
  if (q === 1) return '1 usuário RH extra - Recruta Indústria (mensal)';
  return `${q} usuários RH extras - Recruta Indústria (mensal)`;
}
